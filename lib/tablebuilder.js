...
var SQL_CREATE_TABLE = M(function () {/***
                        IF NOT EXISTS (
                            SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '%1$s' AND TABLE_NAME = '%2$s'
                        ) CREATE TABLE [%1$s].[%2$s] (%3$s)
                        ***/}),
    SQL_DEF_ID_STRING = M(function () {/***
                        [id] NVARCHAR(255) PRIMARY KEY NONCLUSTERED DEFAULT CONVERT(nvarchar(255), NEWID())
                        ***/}),
    SQL_DEF_ID_INT = M(function () {/***
